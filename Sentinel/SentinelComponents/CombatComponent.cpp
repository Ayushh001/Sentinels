...
#include "Sentinel/Character/SentinelAnimInstance.h"
#include "Sentinel/Weapon/Projectile.h"
#include "Sentinel/Character/SentinelAnimInstance.h"
#include "Sentinel/Weapon/Projectile.h"
#include "Sentinel/Weapon/Shotgun.h"
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		HUD = HUD == nullptr ? Cast<ASentinelHUD>(Controller->GetHUD()) : HUD;
		if (HUD)
		{
			if (EquippedWeapon)
			{
...
bool UCombatComponent::ServerFire_Validate(const FVector_NetQuantize& TraceHitTarget, float FireDelay)
{
	if (EquippedWeapon)
	{
		bool bNealyEqual = FMath::IsNearlyEqual(EquippedWeapon->FireDelay, FireDelay, 0.001f);
		return bNealyEqual;
	}
	return true;
}
...
void UCombatComponent::ThrowGrenade()
{
	...
	if (Character && Character->HasAuthority())
	{
		Grenades = FMath::Clamp( -1, 0, MaxGrenades);
		UpdateHUDGrenades();
	}
}
